...
<title><c:choose><c:when test="${requestScope.searchValue != null}">${requestScope.searchValue}</c:when>
    <c:otherwise><c:choose><c:when
            test="${requestScope.productList != null && fn:length(requestScope.productList)>0}">${requestScope.productList[0].product_category.category_name}</c:when><c:otherwise>没找到相关商品</c:otherwise></c:choose></c:otherwise></c:choose>-天猫</title>
<nav>
...
<div class="context">
    <c:choose>
                ...
                <c:forEach items="${requestScope.productList}" var="product">
                    <div class="context_productStyle">
                        <div class="context_product">
                            ...
                            <p class="context_product_shop"><span>XF${product.product_category.category_name}旗舰店</span>
                            </p>
        ...
        <c:otherwise>
            <div class="error">
                <h2>没找到与“${requestScope.searchValue}”相关的 商品 </h2>
                <ol>
                    <li>
                        <form action="${pageContext.request.contextPath}/product" method="get">
